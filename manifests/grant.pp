...
define postgresplus::grant (
  ...
  if($role != undef and defined(Postgresplus::Role[$role])) {
    Postgresplus::Role[$role]->Postgresql_psql[$grant_cmd]
  }

  if($db != undef and defined(Postgresplus::Database[$db])) {
    Postgresplus::Database[$db]->Postgresql_psql[$grant_cmd]
  }
}
